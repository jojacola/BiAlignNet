...
parser.add_argument("--biseg", action="store_true", default=False, help="start biseg loss")
...
parser.add_argument('--mybiseg', action='store_true', default=False, help="whether use the loss")
parser.add_argument('--edge3aux', action='store_true', default=False, help="whether use the loss")
...
def train(train_loader, net, optim, curr_epoch, writer):
    ...
    for i, data in enumerate(train_loader):
        ...
        if args.joint_edgeseg_loss:
            ...
            main_loss = sum([loss for loss in main_loss_dic.values()])
        ...
        if args.local_rank == 0:
            if args.joint_edgeseg_loss:
                msg = '[epoch {}], [iter {} / {}]'.format(curr_epoch, i + 1, len(train_loader))
                for key in main_loss_dic:
                    msg += ', [{} {:0.6f}]'.format(key, main_loss_dic[key])
                msg += ', [lr {:0.6f}]'.format(optim.param_groups[-1]['lr'])
