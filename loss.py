...
def get_loss(args):
    ...
    if args.img_wt_loss:
        ...
        criterion = MyBiSegLoss(classes=args.dataset_cls.num_classes,
                                         ignore_index=args.dataset_cls.ignore_label,
                                         edge_weight=args.edge_weight, seg_weight=args.seg_weight,
                                         att_weight=args.att_weight).cuda()
    elif args.edge3aux:
        criterion = CriterionEdge3Aux(classes=args.dataset_cls.num_classes,
                                         ignore_index=args.dataset_cls.ignore_label,
                                         edge_weight=args.edge_weight, seg_weight=args.seg_weight,
                                         att_weight=args.att_weight).cuda()
    else:
        if args.ohem and args.aux:
            ...
            criterion = CriterionBiSegNet(ignore_index=args.dataset_cls.ignore_label).cuda()
        else:
...
class MyBiSegLoss(nn.Module):
    def __init__(self, classes, ignore_index=255,mode='train',
                 edge_weight=1, seg_weight=1, seg_body_weight=1, att_weight=1):
        super(MyBiSegLoss, self).__init__()
        self.num_classes = classes
        if mode == 'train':
            self.seg_loss = OhemCrossEntropy2dTensor(ignore_index=ignore_index).cuda()
        elif mode == 'val':
    ...
    def forward(self, inputs, targets):
        ...
        mask, edgemask = targets
        losses = {}
        # print("seg_in: ", seg_in.shape)
        # print("edge_in: ", edge_in.shape)
        # print("mask: ", mask.shape)
        # print("edgemask: ", edgemask.shape)

        losses['seg_loss'] = self.seg_weight * self.seg_loss(seg_in, mask)
        losses['edge_loss'] = self.edge_weight * 25 * self.bce2d(edge_in, edgemask)
        losses['edge_ohem_loss'] = self.att_weight * self.edge_attention(seg_in, mask, edge_in)

        return losses

class CriterionEdge3Aux(nn.Module):
    def __init__(self, classes, ignore_index=255,mode='train',
                 edge_weight=1, seg_weight=1, seg_body_weight=1, att_weight=1):
        super(CriterionEdge3Aux, self).__init__()
        self.num_classes = classes
        if mode == 'train':
            self.seg_loss = OhemCrossEntropy2dTensor(ignore_index=ignore_index).cuda()
        elif mode == 'val':
    ...
    def forward(self, inputs, targets):
        seg_in, edge_in, cxt_aux, cxt_aux2 = inputs
        mask, edgemask = targets
        losses = {}
        h, w = mask.size(1), mask.size(2)

        losses['seg_loss'] = self.seg_weight * self.seg_loss(seg_in, mask)
        losses['cxt_16_seg_loss'] = 0.3 * self.seg_loss(cxt_aux2, mask)
        losses['cxt_8_seg_loss'] = 0.3 * self.seg_loss(cxt_aux, mask)
        losses['edge_loss'] = self.edge_weight * 25 * self.bce2d(edge_in, edgemask)
        losses['edge_ohem_loss'] = self.att_weight * self.edge_attention(seg_in, mask, edge_in)
