...
from network.nn.operators import ConvBNReLU, conv_sigmoid, _AtrousSpatialPyramidPoolingModule, BiSeNetOutput, BiSeNetOutput2
...
class BiAlignNet(nn.Module):
    def __init__(self, in_planes=256, inner_planes=128, backbone='dfv2', gate="", aux=False, edgemap=True, num_classes=19, sync_bn=True, criterion=None):
    ...
    def forward(self, x, gts=None):
        ...
        if self.training:
            if self.edgemap:
                spatial_out = self.spatial_out(spatial_fuse)
                spatial_out = self.sigmoid_spatial(spatial_out)
                if self.aux:
                    cxt16_aux_out =  F.upsample(self.cxt16_aux(feat16), size=size[2:], mode='bilinear', align_corners=True)
                    cxt8_aux_out =  F.upsample(self.cxt8_aux(feat8), size=size[2:], mode='bilinear', align_corners=True)
                    return self.criterion((final_out, spatial_out, cxt8_aux_out, cxt16_aux_out), gts)
                return self.criterion((final_out, spatial_out), gts)
            else:
