...
class dfnetMini(nn.Module):
    def __init__(self, num_classes=1000, norm_layer=nn.BatchNorm2d, stride=32):
        super(dfnetMini, self).__init__()
        self.inplanes = 64
        self.out_planes = 512
        self.stage1 = nn.Sequential(
            nn.Conv2d(3, 32, kernel_size=3, padding=1, stride=2, bias=False),
        ...
        self.stage2 = self._make_layer(64, 1, stride=2, normal_layer=norm_layer)
        self.stage3 = self._make_layer(128, 1, stride=2, normal_layer=norm_layer)
        self.stage4 = self._make_layer(256, 1, stride=2, normal_layer=norm_layer)
        if stride == 32:
            self.stage5 = self._make_layer(512, 1, stride=1, normal_layer=norm_layer)
        elif stride ==64:
            self.stage5 = self._make_layer(512, 1, stride=2, normal_layer=norm_layer)
...
class dfnetMini2(nn.Module):
    def __init__(self, num_classes=1000, norm_layer=nn.BatchNorm2d, stride=32):
        super(dfnetMini2, self).__init__()
        self.inplanes = 64
        self.out_planes = 512
        self.stage1 = nn.Sequential(
            nn.Conv2d(3, 32, kernel_size=3, padding=1, stride=2, bias=False),
        ...
        self.stage2 = self._make_layer(64, 2, stride=2, normal_layer=norm_layer)
        self.stage3 = self._make_layer(128, 2, stride=2, normal_layer=norm_layer)
        self.stage4 = self._make_layer(256, 2, stride=2, normal_layer=norm_layer)
        if stride == 32:
            self.stage5 = self._make_layer(256, 1, stride=1, normal_layer=norm_layer)
        elif stride ==64:
            self.stage5 = self._make_layer(512, 1, stride=2, normal_layer=norm_layer)
        else:
            raise ValueError("stride must be 32 or 64")
...
class dfnetMini3(nn.Module):
    def __init__(self, num_classes=1000, norm_layer=nn.BatchNorm2d, stride=32):
        super(dfnetMini3, self).__init__()
        self.inplanes = 64
        self.out_planes = 512
        self.stage1 = nn.Sequential(
            nn.Conv2d(3, 32, kernel_size=3, padding=1, stride=2, bias=False),
...
class dfnetMini4(nn.Module):
    def __init__(self, num_classes=1000, norm_layer=nn.BatchNorm2d, stride=32):
        super(dfnetMini4, self).__init__()
        self.inplanes = 64
        self.out_planes = 512
        self.stage1 = nn.Sequential(
            nn.Conv2d(3, 32, kernel_size=3, padding=1, stride=2, bias=False),
...
class dfnetMini5(nn.Module):
    def __init__(self, num_classes=1000, norm_layer=nn.BatchNorm2d, stride=32):
        super(dfnetMini5, self).__init__()
        self.inplanes = 64
        self.out_planes = 512
        self.stage1 = nn.Sequential(
            nn.Conv2d(3, 32, kernel_size=3, padding=1, stride=2, bias=False),
...
class dfnetMini6(nn.Module):
    def __init__(self, num_classes=1000, norm_layer=nn.BatchNorm2d, stride=32):
        super(dfnetMini6, self).__init__()
        self.inplanes = 64
        self.out_planes = 512
        self.stage1 = nn.Sequential(
            nn.Conv2d(3, 32, kernel_size=3, padding=1, stride=2, bias=False),
...
def DFNetMini(pretrained=False, **kwargs):
    """
        Init model
    """
    model = dfnetMini(**kwargs)
    return model


def DFNetMini2(pretrained=False, **kwargs):
    """
        Init model
    """
    model = dfnetMini2(**kwargs)
    return model

def DFNetMini3(pretrained=False, **kwargs):
    """
        Init model
    """
    model = dfnetMini3(**kwargs)
    return model

def DFNetMini4(pretrained=False, **kwargs):
    """
        Init model
    """
    model = dfnetMini4(**kwargs)
    return model

def DFNetMini5(pretrained=False, **kwargs):
    """
        Init model
    """
    model = dfnetMini5(**kwargs)
    return model

def DFNetMini6(pretrained=False, **kwargs):
    """
        Init model
    """
    model = dfnetMini6(**kwargs)
    return model



class dfnetv1test(nn.Module):
    def __init__(self, num_classes=1000, norm_layer=nn.BatchNorm2d, stride=32):
        super(dfnetv1test, self).__init__()
        self.inplanes = 64
        self.out_planes = 512
        self.stage1 = nn.Sequential(
            nn.Conv2d(3, 32, kernel_size=3, padding=1, stride=2, bias=False),
...
def DFNetv1Test(pretrained=True, **kwargs):
    """
        Init model
    """
    model = dfnetv1test(**kwargs)
    if pretrained:
        model.load_state_dict(torch.load(model_urls['dfv1'], map_location="cpu"), strict=False)
    return model