...
def make_dataset(quality, mode):
    ...
    if quality == 'semantic':
        ...
        mask_path = os.path.join(root, img_dir_name, 'instances19_2048')
    else:
        raise BaseException("Instance Segmentation Not support")

    img_path = os.path.join(root, img_dir_name, 'images2048')
    print(img_path)
    if quality != 'video':
...
class Mapillary(data.Dataset):
    def __init__(self, quality, mode, joint_transform_list=None,
                 ...
                 class_uniform_pct=0, class_uniform_tile=768, test=False, edge_map=False):
        """
    ...
    def __getitem__(self, index):
        ...
        if self.edge_map:
            _edgemap = edge_utils.label_to_binary_edges(mask, num_classes, 2)
