...
def calc_tile_locations(tile_size, image_size):
    ...
    if isinstance(tile_size, numbers.Number):
        tile_size_y, tile_size_x = int(tile_size), int(tile_size)
    else:
        tile_size_y, tile_size_x = tile_size
    image_size_y, image_size_x = image_size
    locations = []
    for y in range(image_size_y // tile_size_y):
        for x in range(image_size_x // image_size_x):
            x_offs = x * image_size_x
            y_offs = y * tile_size_y
...
def class_centroids_image(item, tile_size, num_classes, id2trainid):
    ...
    for x_offs, y_offs in tile_locations:
        patch = mask[y_offs:y_offs + tile_size_y, x_offs:x_offs + tile_size_x]
